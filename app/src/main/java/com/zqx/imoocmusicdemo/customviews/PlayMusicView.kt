...
class PlayMusicView : FrameLayout {
    ...
    private var mMediaPlayerHelper: MediaPlayerHelper? = null
    ...
    private fun init(context: Context) {
        ...
        mPlayMusicAnim = ObjectAnimator.ofFloat(fl_play_music, "rotation", 0f, 360f)
        ...
        mPlayNeedleAnim = ObjectAnimator.ofFloat(iv_play_music_needle, "rotation", -20f, 0f)
        ...
        mStopNeedleAnim = ObjectAnimator.ofFloat(iv_play_music_needle, "rotation", 0f, -20f)
        ...
        mMediaPlayerHelper = MediaPlayerHelper.getInstance(context)
    ...
    fun playMusic(path: String) {
        ...
        val currentPath = mMediaPlayerHelper?.getPath()
        if (currentPath != null && currentPath == path
        ) {
            mMediaPlayerHelper?.start()
        } else {
            mMediaPlayerHelper?.setPath(path)
            mMediaPlayerHelper?.setOnMediaPlayerHelperListener {
                it.start()
            }
        }
    }
